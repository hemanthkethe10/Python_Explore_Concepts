...
pinecone_client = Pinecone(api_key="a95d974d-5df3-4d7d-8b1e-f5e2927ee1cb", environment="us-east-1")
...
def process_and_insert(document_content, metadata):
    ...
    for i, chunk in enumerate(chunks):
        ...
        chunk_metadata.update({"originalText": chunk})
        
        # Upsert into Pinecone
        index.upsert(vectors=[
            {"id": f"{metadata['source']}_chunk_{i}", "values": embedding, "metadata": chunk_metadata}
        ],namespace='default')
